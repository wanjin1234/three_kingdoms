...
> 一款基于 **Python + Pygame** 的回合制六边形棋盘策略游戏，以三国时期为背景，玩家可选择魏、蜀、吴三国之一，与 AI 对手在中原大地上展开争霸。（本文档部分内容由AI生成）
...
前往本项目的 **[GitHub Releases 页面](../../releases)**，下载最新版本的 `three_kingdoms.v1.x.x.exe`，双击即可运行，无需额外安装步骤。
...
| ---------------- | ------------------------------------------- |
| `pygame`         | 图形渲染、事件处理、音频                    |
| `PyMuPDF (fitz)` | 在游戏内渲染 `rules.pdf` 规则书为可翻页图像 |

> **规则书**：游戏内按帮助键可查看规则书，规则内容存放于根目录的 `rules.pdf`，由 PyMuPDF 在运行时逐页渲染为图像后显示。
...
├── rules.pdf                  ← 游戏规则书（在游戏内按帮助键查看）
├── icon.ico                   ← 游戏窗口图标
├── three_kingdoms.spec        ← PyInstaller 打包配置（用于生成 EXE）
│
├── assets/                    ← 所有静态资源
│   ├── data/                  ← JSON 数据配置
