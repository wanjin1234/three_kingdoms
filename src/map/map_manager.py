...
class MapManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._provinces_map: Dict[int, Province] = {
            p.province_id: p for p in self._provinces_list
        }
    ...
    def draw(self, surface: pg.Surface) -> None:
        ...
        if not self._hex_side:
            raise RuntimeError(
                "Hex side length has not been initialized (格子边长未初始化)"
            )

        # 检查缓存是否有效 (存在且尺寸匹配)
        if (
            self._cached_background is None
            or self._cached_background.get_size() != surface.get_size()
        ):
            # 创建新的缓存层 (带透明通道)
            ...
            for province in self._provinces_list:
                ...
                # 4. 画国家颜色的底色（填充，75%透明度 = 25%不透明）
                fill_color = pg.Color(
                    color.r, color.g, color.b, 64
                )  # alpha=64 为25%不透明度（75%透明）
                pg.draw.polygon(self._cached_background, fill_color, vertices)
                # 5. 画灰色的边框（50%透明度的黑色）
                border_color = pg.Color(
                    0, 0, 0, 128
                )  # 灰色边框，50%不透明度（50%透明）
                self._draw_hex_border(
                    self._cached_background, border_color, vertices, self._border_width
                )
                # 6. 画地形图标 (山、城等)
                self._draw_terrain_icon(
                    self._cached_background, province.terrain, center
                )
    ...
    def _draw_hex_border(
        ...
        surface: pg.Surface,
        color: pg.Color,
        vertices: Sequence[pg.math.Vector2],
        width: int,
    ) -> None:
        """
    ...
    def _draw_terrain_icon(
        self, surface: pg.Surface, terrain: str, center: pg.math.Vector2
    ) -> None:
        """绘制地形图标，放在格子的左上角"""
        if not terrain:
            return
            ...
            for fname in candidates:
                ...
                if fpath.exists():
                    try:
                        ...
                        try:
                            loaded_surface = pg.transform.smoothscale(
                                surf, (target_size, target_size)
                            )
                        except Exception as e:
                            print(
                                f"smoothscale failed for {fname}, falling back to scale: {e}"
                            )
                            loaded_surface = pg.transform.scale(
                                surf, (target_size, target_size)
                            )
