...
class CardEffectManager:
    ...
    current_effects: Dict[str, ProvinceEffect] = field(
        default_factory=dict
    )  # province_id -> effect
    active_offensive_cards: List[str] = field(
        default_factory=list
    )  # 已激活的offensive卡牌ID

    def add_effect(
        self, province_id: str, card_id: str, card_name: str, effect: ProvinceEffect
    ) -> None:
        """添加卡牌效果到指定格子"""
    ...
    def apply_card_effect(
        self, card_id: str, card_name: str, province_id: str, country: str
    ) -> bool:
        """
        应用卡牌效果。
        ...
        # and therefore is NOT handled here.
            ...
            card_id == "card_huoshao_lianying"
        ):  # 火烧连营 (吴国) - offensive，针对堆叠部队
