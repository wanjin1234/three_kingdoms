...
SPECIAL_LOCATIONS = {
    ...
    "Jingzhou": (2, "SHU"),    # 荆州 2 分
    "Chengdu": (5, "SHU"),     # 成都 5 分
    # 曹魏
    "Liangzhou": (2, "WEI"),   # 凉州 2 分
...
class ScoreManager:
    ...
    def calculate_province_score(self, province) -> float:
        ...
        """
        name = province.name
        special = SPECIAL_LOCATIONS.get(name)
        
        if special:
            return float(special[0])
        
        # 普通平原、山地计 0.5 分
        return NORMAL_TERRAIN_SCORE
    ...
    def get_net_scores(
        self, 
        provinces: List,
        country_stats: Dict[str, Dict[str, int]] | None = None
    ) -> Dict[str, float]:
        """
        计算净得分（当前分数 - 开局分数）。
        ...
        for country in ["SHU", "WEI", "WU"]:
            net[country] = current.get(country, 0.0) - self.initial_scores.get(country, 0.0)
    ...
    def get_detailed_scores(
        ...
        for prov in provinces:
            ...
            # 记录特殊地点
            if prov.name in SPECIAL_LOCATIONS:
                special_score, owner = SPECIAL_LOCATIONS[prov.name]
                if prov.country == "SHU":
